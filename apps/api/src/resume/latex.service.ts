...
export class LatexService {
  ...
  async generateLatex(
    ...
    // Title line
    const title = candidateProfile?.profile?.desiredTitle || candidateProfile?.experiences?.[0]?.title;
    if (title) {
    ...
    if (resumeData.education && resumeData.education.length > 0) {
      ...
      const eduBlocks = resumeData.education.map((edu) => {
        ...
        const dates = matchedProfileEdu?.endDate ? ` \\hfill \\textit{${this.escapeLatex(matchedProfileEdu.endDate)}}` : '';
    ...
    if (resumeData.certifications && resumeData.certifications.length > 0) {
      ...
      const certBlocks = resumeData.certifications.map((cert) => {
        ...
        const issuer = matchedProfileCert?.issuer ? ` — ${this.escapeLatex(matchedProfileCert.issuer)}` : '';
        const date = matchedProfileCert?.date ? ` \\hfill \\textit{${this.escapeLatex(matchedProfileCert.date)}}` : '';
