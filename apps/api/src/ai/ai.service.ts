...
export class AiService {
  ...
  private readonly model = process.env.AI_MODEL || 'groq/compound-mini';
  ...
  async runStructuredCall<T>(params: StructuredCallParams<T>): Promise<T> {
    ...
    if (this.apiKey) {
      this.logger.log(`Running structured call [${schemaName}] using model: ${this.model}`);
  ...
  private async callLlmWithRetry<T>(
    ...
    if (!response.ok) {
      ...
      // If rate limited (429), parse delay or wait and retry
      if (response.status === 429 && remainingRetries > 0) {
        ...
        this.logger.warn(
          `Rate limit (429) hit on [${schemaName}]. Auto-waiting ${waitMs}ms before retry...`,
        );
    ...
    try {
      ...
      if (remainingRetries > 0) {
        this.logger.warn(
          `Schema validation failed for [${schemaName}], retrying with error feedback: ${errors}`,
        );
  ...
  private fallbackDeterministicEngine<T>(params: StructuredCallParams<T>): T {
    ...
    if (schemaName === 'Resume') {
      ...
      // Skills: ONLY candidate skills that are EXPERIENCED or WORKING_KNOWLEDGE
      ...
      const resumeData = {
        ...
        skills: allowedSkills.slice(0, 16),
