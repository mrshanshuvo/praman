    ...
    <div className="w-full overflow-x-auto pb-4">
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 min-w-255">
        {KANBAN_COLUMNS.map((col) => (
          <div
            key={col.key}
            className="flex flex-col rounded-2xl border border-border/80 bg-card/40 min-h-125"
          >
            ...
            >
              <div className="flex items-center gap-2">
                <div className={`p-1 rounded-lg border ${col.accentColor} opacity-70`}>
              ...
              <div className="w-5 h-4 rounded bg-muted/60" />
            </div>

            {/* Column Cards Skeleton */}
            <div className="flex-1 p-2.5 space-y-2.5">
              {[1, 2].map((i) => (
                <div key={i} className="p-3.5 rounded-xl border border-border bg-card/80 space-y-3">
